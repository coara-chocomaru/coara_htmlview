...
Java_com_coara_htmlview_htmlview_getHighlightSpansNative(JNIEnv *env, jobject obj, jstring jtext, jint scrollY, jint height, jintArray lineStarts, jintArray lineEnds, jintArray lineTops) {
    ...
    if (isLargeText) {
        jsize numLines = (*env)->GetArrayLength(env, lineStarts);
        if (numLines != (*env)->GetArrayLength(env, lineEnds) || numLines != (*env)->GetArrayLength(env, lineTops)) {
            // Arrays must be of same length; handle error
            (*env)->ReleaseStringUTFChars(env, jtext, text);
            return NULL;
        }

        jint* starts = (*env)->GetIntArrayElements(env, lineStarts, NULL);
        jint* ends = (*env)->GetIntArrayElements(env, lineEnds, NULL);
        jint* tops = (*env)->GetIntArrayElements(env, lineTops, NULL);

        if (starts == NULL || ends == NULL || tops == NULL) {
            if (starts != NULL) (*env)->ReleaseIntArrayElements(env, lineStarts, starts, JNI_ABORT);
            if (ends != NULL) (*env)->ReleaseIntArrayElements(env, lineEnds, ends, JNI_ABORT);
            if (tops != NULL) (*env)->ReleaseIntArrayElements(env, lineTops, tops, JNI_ABORT);
            (*env)->ReleaseStringUTFChars(env, jtext, text);
            return NULL;
        }
        ...
        for (jsize i = 0; i < numLines; i++) {
            int lineTop = tops[i];
            int lineHeight = (i + 1 < numLines) ? (tops[i + 1] - tops[i]) : 20;  // Assume default height for last line
        ...
        if (firstVisibleLine != -1) {
            // Add buffer: 5 lines before and after for smoother scrolling
            int bufferedFirst = (firstVisibleLine > 5) ? (firstVisibleLine - 5) : 0;
            int bufferedLast = (lastVisibleLine + 5 < numLines - 1) ? (lastVisibleLine + 5) : (numLines - 1);

            visibleStart = starts[bufferedFirst];
            visibleEnd = ends[bufferedLast];
        } else {
            // No visible lines; default to full text
            visibleStart = 0;
            visibleEnd = (int)text_len;
        }

        // Ensure visibleStart and visibleEnd are within bounds
        if (visibleStart < 0) visibleStart = 0;
        if (visibleEnd > (int)text_len) visibleEnd = (int)text_len;

        (*env)->ReleaseIntArrayElements(env, lineStarts, starts, JNI_ABORT);
        (*env)->ReleaseIntArrayElements(env, lineEnds, ends, JNI_ABORT);
        (*env)->ReleaseIntArrayElements(env, lineTops, tops, JNI_ABORT);
    }
    ...
    result = (*env)->NewObjectArray(env, (jsize)spans_count, int_array_class, NULL);
    if (result == NULL) {
        goto cleanup;
    }
